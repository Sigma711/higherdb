...
pub trait DB {
    ...
    /// `close` shuts down the current HigherDB by waiting util all the background tasks are complete
    /// and then releases the file lock. A closed db should never be used again and is able to be
    /// dropped safely.
    fn close(&mut self) -> Result<()>;

    /// `destroy` shuts down the current HigherDB and delete all relative files and the db directory.
...
/// `HigherDB` is thread safe and is able to be shared by `clone()` in different threads.
#[derive(Clone)]
pub struct HigherDB<S: Storage + Clone + 'static, C: Comparator> {
...
pub type HigherDBIterator<S, C> = DBIterator<InternalIterator<S, C>, S, C>;
...
impl<S: Storage + Clone, C: Comparator + 'static> DB for HigherDB<S, C> {
    type Iterator = HigherDBIterator<S, C>;
...
impl<S: Storage + Clone, C: Comparator + 'static> HigherDB<S, C> {
    /// Create a new HigherDB
...
mod tests {
    ...
    struct DBTest {
        ...
        db: HigherDB<MemStorage, BytewiseComparator>,
    }
    ...
    impl DBTest {
        fn new(opt: Options<BytewiseComparator>) -> Self {
            ...
            let name = "db_test";
            let db = HigherDB::open_db(opt.clone(), name, store.clone()).unwrap();
            DBTest { store, opt, db }
        }

        // Close the inner db without destroy the contents and establish a new HigherDB on same db path with same option
        fn reopen(&mut self) -> Result<()> {
            self.db.close()?;
            let db =
                HigherDB::open_db(self.opt.clone(), &self.db.inner.db_path, self.store.clone())?;
            self.db = db;
            Ok(())
        }
    ...
    impl Deref for DBTest {
        type Target = HigherDB<MemStorage, BytewiseComparator>;
    ...
    fn test_comparator_check() {
        ...
        let mut db = HigherDB::open_db(opts, "test", storage.clone()).unwrap();
        db.close().unwrap();
        let opts = Options::<BytewiseComparator>::default();
        let res = HigherDB::open_db(opts, "test", storage.clone());
        match res {
    ...
    fn test_custom_comparator() {
        ...
        let store = MemStorage::default();
        let db = HigherDB::open_db(opts, "test", store).unwrap();
    ...
    fn test_dbopen_options() {
        ...
        match HigherDB::open_db(opts.clone(), dbname, store.clone()) {
        ...
        opts.create_if_missing = true;
        let mut db = HigherDB::open_db(opts.clone(), dbname, store.clone()).unwrap();
        db.close().unwrap();
        ...
        let _ = HigherDB::open_db(opts, dbname, store.clone()).unwrap();
    }
    ...
    fn test_destroy_empty_dir() {
        ...
        let mut db = HigherDB::open_db(opts, dbname, store.clone()).unwrap();
    ...
    fn test_db_file_lock() {
        ...
        let _ = HigherDB::open_db(opts.clone(), dbname, store.clone()).unwrap();
        match HigherDB::open_db(opts, dbname, store.clone()) {
    ...
    fn test_db_reads_using_bloom_filter() {
        ...
        let db = HigherDB::open_db(opts, "bloom_filter_test", store.clone()).unwrap();
