...
/// A Comparator implementation must be thread-safe since we may invoke
/// its methods concurrently from multiple threads.
pub trait Comparator: Send + Sync + Clone + Default {
  ...
  /// comparator implementation changes in a way that will cause the relative
  /// ordering of any two keys to change.
  fn name(&self) -> &str;

  /// Given feasible keys a, b for which Compare(a, b) < 0, Separator returns
  /// a feasible key k such that:
  /// 1. Compare(a, k) <= 0, and
  /// 2. Compare(k, b) < 0.
  /// `separator` is used to construct SSTable index blocks. A trivial
  /// implementation is `return a`, but appending fewer bytes leads to smaller
  /// SSTables.
