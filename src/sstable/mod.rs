...
mod tests {
    ...
    use crate::db::{HigherDB, HigherDBIterator, DB};
    ...
    struct DBConstructor {
        inner: HigherDB<MemStorage, TestComparator>,
    }

    struct DBIterWrapper {
        inner: HigherDBIterator<MemStorage, TestComparator>,
        key_buf: Vec<u8>,
        value_buf: Vec<u8>,
    }
    impl DBIterWrapper {
    ...
    impl Constructor for DBConstructor {
        ...
        fn new(is_reversed: bool) -> Self {
            ...
            let db = HigherDB::open_db(options, "table_testdb", env).expect("could not open db");
            Self { inner: db }
        }
